 ...
 return root.banner.listAll;
  ...
  const getBannerList = async ({ page = 1, limit = PER_PAGE_LIMIT }: any) => {
    try {
      ...
      const totalPages = Math.ceil(response.mets.total / response.meta.limit)
      setPagination({
...
toast.success("Banner deledted successfully")
getBannerList({page:1, limit:PER_PAGE_LIMIT})
    }
    catch(exception){
      ...
      <section>
        <div className="mx-auto max-w-screen-xl px-4 py-4 sm:px-6 lg:px-8 ">
          <div className="grid grid-cols-3 gap-8  lg:gap-16">
            ...
            <div></div>
            <NavLink to="/admin/banner/create" className={" bg-green-700 mt-3 text-center py-2 text-whiteb rounded w-[200px]"}>
              Create Banner
            </NavLink>
